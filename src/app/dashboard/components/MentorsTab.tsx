  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        {mentorStatus && (
          <div 
            className="mb-6 rounded-2xl p-6 cursor-pointer hover:-translate-y-0.5 transition-all duration-150"
            style={{ 
          ...
          >
            <h2 className="text-xl font-bold text-white mb-4">You are a mentor!</h2>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-all ${
                    mentorStatus.availability === 'online'
                      ? 'bg-white/20 border-2 border-white text-white backdrop-blur-sm'
                      : 'bg-white/10 border border-white/30 text-white/80 hover:bg-white/15 backdrop-blur-sm'
                  }`}
                >
                  🟢 Online
                </button>
            ...
            <div className="mb-4 text-sm text-white/90">
              <div className="mb-1">Today: <span className="font-bold text-white">{mentorStatus.current_day_points}</span> pts</div>
              <div className="mb-1">Week: <span className="font-bold text-white">{mentorStatus.current_week_points}</span> pts</div>
              <div>Lifetime: <span className="font-bold text-white">{mentorStatus.lifetime_points}</span> pts</div>
            </div>
            ...
            <button
              ...
              className="px-4 py-2 bg-white/20 hover:bg-white/30 border border-white/50 text-white text-sm font-medium rounded-lg transition-colors backdrop-blur-sm"
            >
        ...
        {!mentorStatus && (
          ...
          >
            <h2 className="text-xl font-bold text-white mb-2">Become a Mentor</h2>
            <p className="text-white/80 mb-4">Help others and compete for daily/weekly prizes</p>
            <button
              onClick={() => handleToggleMentor(true)}
              disabled={togglingMentor}
              className="px-6 py-3 bg-white/20 hover:bg-white/30 border border-white/50 text-white font-semibold rounded-lg transition-all backdrop-blur-sm"
            >
        ...
        {leaderboard?.clouted_mentor && (
          ...
          >
            <h2 className="text-lg font-bold text-white mb-4">Clouted Mentor</h2>
            <div className="flex items-center gap-4">
              {leaderboard.clouted_mentor.avatar_url ? (
                ...
                <div className="w-12 h-12 rounded-full bg-white/20 border-2 border-white/50 flex items-center justify-center text-white font-bold">
                  {leaderboard.clouted_mentor.avatar_name?.[0]?.toUpperCase() || leaderboard.clouted_mentor.name[0]?.toUpperCase()}
                </div>
              )}
              <div>
                <div className="text-lg font-bold text-white mb-1">{leaderboard.clouted_mentor.avatar_name || leaderboard.clouted_mentor.name}</div>
                <MentorBadge lifetimePoints={leaderboard.clouted_mentor.lifetime_points} />
                <div className="text-white/70 text-sm mt-1">{leaderboard.clouted_mentor.avatar_name || leaderboard.clouted_mentor.name} was the most helpful mentor yesterday</div>
              </div>
            </div>
          </div>
        )}
        ...
        {leaderboard && (
          ...
          >
              ...
              {editingRaffle && isAdmin ? (
                <div className="flex-1 flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-1 bg-white/20 border border-white/50 rounded-lg text-white text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-white/50 backdrop-blur-sm"
                    autoFocus
                  />
                  <button
                    onClick={() => {
                    ...
                    className="p-1.5 bg-white/20 hover:bg-white/30 border border-white/50 rounded-lg transition-colors backdrop-blur-sm"
                  >
            ...
            <div className="text-white/90 mb-3">
              <div className="mb-1">Your entries: <span className="font-bold text-white">{leaderboard.user_raffle_entries}</span></div>
              <div>Days left: <span className="font-bold text-white">{leaderboard.days_left_in_month}</span></div>
            </div>
        ...
        >
          <h2 className="text-lg font-bold text-white mb-4">🟢 AVAILABLE NOW</h2>
          {onlineMentors.length === 0 ? (
            <div className="text-center py-8 text-white/70">No mentors available right now</div>
          ) : (
            <div className="space-y-3">
              {onlineMentors.map(mentor => (
                <div 
                  key={mentor.id} 
                  className="flex items-center justify-between p-4 bg-white/10 rounded-lg border border-white/20 backdrop-blur-sm hover:bg-white/15 transition-all cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    {mentor.avatar_url ? (
                      <img
                      ...
                      <div className="w-10 h-10 rounded-full bg-white/20 border-2 border-white/30 flex items-center justify-center text-white font-semibold">
                        {(mentor.avatar_name || mentor.name)[0]?.toUpperCase()}
                      </div>
                    )}
                    <div>
                      <div className="text-white font-semibold">{mentor.avatar_name || mentor.name}</div>
                      <MentorBadge lifetimePoints={mentor.lifetime_points} showLabel={false} size="sm" />
                    </div>
                  </div>
                  <button
                    onClick={() => handleAskQuestion(mentor)}
                    className="px-4 py-2 bg-white/20 hover:bg-white/30 border border-white/50 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2 backdrop-blur-sm"
                  >
        ...
        {awayMentors.length > 0 && (
          <div 
            ...
            style={{ 
              ...
              boxShadow: glowShadow('0 0 40px rgba(250,204,21,0.6), 0 0 80px rgba(250,204,21,0.4), 0 20px 40px rgba(217,119,6,0.3)', glowIntensity)
            }}
          >
            <h2 className="text-lg font-bold text-white mb-4">🟡 AWAY</h2>
            <div className="space-y-3">
          ...
          >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all backdrop-blur-sm ${
                  leaderboardTab === 'today'
                    ...
                    : 'bg-white/10 border border-white/30 text-white/80 hover:bg-white/15'
                }`}
              >
                Today
              </button>
            ...
            <div className="space-y-2">
              {(leaderboardTab === 'today' ? leaderboard.today :
                ...
                <div 
                  key={entry.id} 
                  className="flex items-center justify-between p-3 bg-white/10 rounded-lg border border-white/20 backdrop-blur-sm hover:bg-white/15 transition-all"
                >
                  <div className="flex items-center gap-3">
                    <div className="text-white font-bold w-8">#{entry.rank}</div>
                    {entry.avatar_url ? (
                      <img
                      ...
                      <div className="w-8 h-8 rounded-full bg-white/20 border-2 border-white/30 flex items-center justify-center text-white text-xs font-semibold">
                        {(entry.avatar_name || entry.name)[0]?.toUpperCase()}
                      </div>
                    )}
                    <div className="text-white font-semibold">{entry.avatar_name || entry.name}</div>
                    <MentorBadge lifetimePoints={entry.lifetime_points} showLabel={false} size="sm" />
                  </div>
                  <div className="text-white font-bold">{entry.points || entry.lifetime_points} pts</div>
                </div>
              ))}
              {((leaderboardTab === 'today' ? leaderboard.today :
                leaderboardTab === 'this_week' ? leaderboard.this_week :
                leaderboard.all_time).length === 0) && (
                <div className="text-center py-8 text-white/70">No entries yet</div>
              )}
            </div>
          </div>
        )}
